...
export async function getReward() : Promise<any> {
  ...
  const rewards = await MasterChefContract.methods.cakePerBlock().call() /1e18 * 604800 / 3;
...
export async function getAllStats(httpService) : Promise<any> {
  ...
  const poolCount = parseInt(await masterChefContract.methods.poolLength().call(), 10);
  const totalAllocPoints = await masterChefContract.methods.totalAllocPoint().call();

  const poolInfos = await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getBscPoolInfo(masterChefContract, masterChefContractAddress(), x, "pendingCake", myAddress)));
  ...
  await Promise.all(tokenAddresses.map(async (address) => {
    tokens[address] = await getBscToken(address, masterChefContractAddress(), myAddress);
  }));
...
async function getBscPoolInfo(masterChefContract, masterChefAddress, poolIndex, pendingRewardsFunction, myAddress) {  
  const poolInfo = await masterChefContract.methods.poolInfo(poolIndex).call();
  const poolToken = await getBscToken(poolInfo.lpToken, masterChefContractAddress(), myAddress);
  const userInfo = await masterChefContract.methods.userInfo(poolIndex, myAddress).call();
  const pendingRewardTokens = await masterChefContract.methods.pendingCake(poolIndex, myAddress).call();
  ...
  if (poolInfo.stakedHoldableToken != null && 
    ...
    stakedToken = await getBscToken(poolInfo.stakedHoldableToken, masterChefAddress, myAddress);
...
async function getBep20(token, address, stakingAddress, userAddress) {
  ...
  return {
      ...
      decimals : decimals,
      staked:  await token.methods.balanceOf(stakingAddress).call() / 10 ** decimals,
      unstaked: await token.methods.balanceOf(userAddress).call()  / 10 ** decimals,
      contract: token,
      tokens : [address]
  };
}
...
async function getBscUniPool(pool, poolAddress, stakingAddress, userAddress) {    
  ...
  return { 
      ...
      staked: await pool.methods.balanceOf(stakingAddress).call() / 10 ** decimals,
      decimals: decimals,
      unstaked: await pool.methods.balanceOf(userAddress).call() / 10 ** decimals,
      contract: pool,
