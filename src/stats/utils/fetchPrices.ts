...
export const fetchPrices = async (chainId) => {
  const apePriceGetterAddress = apePriceGetter();
  const tokensToCall = Object.keys(tokens).filter(
    (token) => tokens[token].address[chainId] !== undefined,
  );
  ...
  const tokenDecimals = await multicall(ERC20_ABI, erc20Calls);
